...
#endif
const unichar ApiISO8601DefaultTimeSeparatorCharacter = DEFAULT_TIME_SEPARATOR;
...
@interface ApiISO8601DateFormatter ()
...
@interface ApiISO8601DateFormatter(UnparsingPrivate)
...
@implementation ApiISO8601DateFormatter
...
- (id) init {
	if ((self = [super init])) {
		_parsingCalendar = [self makeCalendarWithDesiredConfiguration];
		_unparsingCalendar = [self makeCalendarWithDesiredConfiguration];

		_format = ApiISO8601DateFormatCalendar;
		_timeSeparator = ApiISO8601DefaultTimeSeparatorCharacter;
		_includeTime = NO;
		_parsesStrictly = NO;
...
- (NSDateComponents *) dateComponentsFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange fractionOfSecond:(out NSTimeInterval *)outFractionOfSecond {
	...
	if (strict) timeSep = ApiISO8601DefaultTimeSeparatorCharacter;
...
- (NSDate *) dateFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange {
	...
	NSDate *parsedDate = [self.parsingCalendar dateFromComponents:components];
...
- (NSString *) stringFromDate:(NSDate *)date formatString:(NSString *)dateFormat timeZone:(NSTimeZone *)timeZone {
	...
	if ([dateFormat isEqualToString:self.lastUsedFormatString] == NO) {
	...
	self.unparsingFormatter.timeZone = timeZone;
	...
	self.unparsingFormatter.timeZone = self.defaultTimeZone;
...
- (NSString *) weekDateStringForDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	...
	if(self.includeTime) {
		...
		unichar timeSep = self.timeSeparator;
		if (!timeSep) timeSep = ApiISO8601DefaultTimeSeparatorCharacter;
